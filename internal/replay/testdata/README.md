...
| `words-consistency-chars` | The server-only metrics, pinned. Every other vector compares only what the client reports (wpm/raw/acc, score); this one's `expect.metrics` block also pins `consistency` (per-second raw WPM through kogasa, on [0, 1]) and the full chars breakdown, exactly as V8 computed them. The run swings its cadence word to word and commits one word short (missed), one with an uncorrected typo (incorrect) and one over-typed (extra), so every count is non-zero and the variance is non-trivial — the coverage test enforces both. |
