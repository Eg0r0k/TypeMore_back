...
- **Results.** A client sends `finish` when done; a finished player then
  **waits** while the others race. A run that ends on a freemod rule (master /
  ...
  finish and fail alike — and the player spectates the rest; the fail itself is
  proven client-side by replaying that player's relayed log, and it ranks them
  below every true finisher. The match ends once every seat is
  finished/dnf/left, at the hard deadline (unfinished ⇒ `dnf`), or when the
  words-mode finish window closes. **AFK rules** (server-owned, PROTOCOL.md §6):
  a racing seat silent for **15 s** is `dnf`'d in EVERY mode (the rule a player
...
- **Reload = forfeit.** A page reload keeps the seat (resume token) but destroys
  the run (event log, `seq`, the t=0 anchor). `room_state.match` tells the fresh
  client a match is in flight; it answers with `finish` at once, so the others
  are never left waiting on a run that can no longer arrive.
