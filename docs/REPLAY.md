...
| `superhuman-burst` | 0.80 | Above the WPM ceiling **for the run's own duration** — 290 wpm at 15 s, falling to 260 at 60 s and 230 at ten minutes (`BURST_CEILING` in the core, interpolated between the anchors, calibrated against the published records of 318 at 15 s and 281 at 60 s). Accuracy is NOT part of the test: it scales severity through a curve that bottoms out at 0.25 rather than at zero, so a fast run made sloppy on purpose still counts. Severity is speed against a fixed 500 wpm scale, so a short genuine burst stays well under the threshold. |
